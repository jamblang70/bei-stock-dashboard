...
function directionStyle(dir: Direction): { dot: string; text: string } {
  switch (dir) {
    case "better": return { dot: "bg-emerald-500", text: "text-emerald-400" };
    case "worse": return { dot: "bg-red-500", text: "text-red-400" };
    default: return { dot: "bg-gray-500", text: "text-text-secondary" };
  }
}
  ...
  return (
    <div className="flex items-center justify-between py-2.5 text-sm">
      ...
      <div className="flex items-center gap-4 text-right">
        <div>
          <p className="text-xs text-text-muted">Emiten</p>
          <p className={`font-semibold ${stockValue === null ? "text-text-muted" : style.text}`}>
            {fmt(stockValue, decimals, suffix)}
          </p>
        </div>
        <div>
          <p className="text-xs text-text-muted">Median Sektor</p>
          <p className={`font-medium ${sectorValue === null ? "text-text-muted" : "text-text-secondary"}`}>
  ...
  if (!data) {
    return (
      <div className="rounded-xl border border-dark-border bg-dark-surface p-6">
        <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-text-secondary">Perbandingan Sektor</h2>
        <p className="text-sm text-text-muted">Data sektor tidak cukup untuk perbandingan</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-dark-border bg-dark-surface p-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-text-secondary">Perbandingan Sektor</h2>
      </div>
      <div className="mb-3 flex gap-4 text-xs text-text-muted">
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Lebih baik</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-red-500" /> Lebih buruk</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-gray-500" /> Netral</span>
      </div>
      ...
      {stockCount && (
        <p className="mt-3 text-xs text-text-muted">Berdasarkan {stockCount} emiten di sektor yang sama</p>
      )}
    </div>
  );
}
